...
function ImageChapter({
  ...
  const frame = useFrameShuffle(src);
  ...
  return (
    <figure className={`image-frame relative ${className}`}>
      ...
      <img key={frame.src} src={frame.src} alt={frame.alt || alt} className={`editorial-image parallax-media motion-picture-image frame-crossfade h-full w-full object-cover ${imageClassName}`} />
...
function ProjectArt({ project }: { project: Project }) {
  const frame = useFrameShuffle(project.image);

  return (
    <div className="project-art relative h-full w-full overflow-hidden bg-[#201a2a]">
      <img key={frame.src} src={frame.src} alt={frame.alt || project.imageAlt} className="motion-picture-image frame-crossfade h-full w-full object-cover" />
      ...
      <section className="relative overflow-hidden border-y border-[#201a2a]/20 bg-[#e76f5c] px-5 py-28 sm:px-8 lg:px-12 lg:py-36" aria-labelledby="capabilities-title">
        <div className="editorial-wrap">
          <div className="flex flex-col justify-between gap-8 sm:flex-row sm:items-end">
            <Reveal><SectionLabel number="03">What we make</SectionLabel></Reveal>
